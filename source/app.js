...
/* Every receiving/stock-out/transfer/adjustment/void gets a short
   TYPE-MMDD-XX code (e.g. "RCV-0915-42") -- no year, since the date's
   already shown right next to it everywhere it's displayed. Leaving the
   year out only works because the backend's duplicate-detection (see
   recentMovementRows in Code.gs) only looks at the last 30 days of the
   ledger, not the whole multi-year history -- so a collision would need
   the same type, same day-of-year, same 2-digit number, AND land within
   30 days of each other, which never happens in practice. */
function shortRef(prefix){
  var mm=TODAY.slice(5,7),dd=TODAY.slice(8,10);
...
/* Near-live updates: Google Sheets has no push mechanism, so this polls
   ...
   'receive' is deliberately NOT in this map: renderReceive() calls
   rcReset(), which snaps the screen back to search mode and clears
   whatever the user was typing — including an in-progress "add new
...
function refreshCurrentScreen(){
  ...
  // a background poll landing mid-keystroke (e.g. typing a count on the
  // Adjust screen) would otherwise steal focus and look like the page
...
var CATICON={
 ...
 'Sunflower seeds':['<svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display:block"><path d="M12 12 C9.8 8.3 9.8 4.7 12 2 C14.2 4.7 14.2 8.3 12 12 Z"/><path d="M12 12 C9.8 8.3 9.8 4.7 12 2 C14.2 4.7 14.2 8.3 12 12 Z" transform="rotate(72 12 12)"/><path d="M12 12 C9.8 8.3 9.8 4.7 12 2 C14.2 4.7 14.2 8.3 12 12 Z" transform="rotate(144 12 12)"/><path d="M12 12 C9.8 8.3 9.8 4.7 12 2 C14.2 4.7 14.2 8.3 12 12 Z" transform="rotate(216 12 12)"/><path d="M12 12 C9.8 8.3 9.8 4.7 12 2 C14.2 4.7 14.2 8.3 12 12 Z" transform="rotate(288 12 12)"/><circle cx="10.8" cy="11.4" r="0.75" fill="currentColor" stroke="none"/><circle cx="13.2" cy="11.4" r="0.75" fill="currentColor" stroke="none"/><circle cx="12" cy="13" r="0.75" fill="currentColor" stroke="none"/></svg>','#fdf3e0','#9a7415'],
 ...
 'Bakery':         ['<svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display:block"><path d="M4 12c0-4.4 3.6-8 8-8s8 3.6 8 8v6a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2v-6z"/><line x1="8" y1="10" x2="7" y2="17"/><line x1="12" y1="9" x2="12" y2="18"/><line x1="16" y1="10" x2="17" y2="17"/></svg>','#f6efe4','#7d5108'],
  ...
  'Kitchenware & Cookware': ['<svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display:block"><path d="M5 9h14v7a3 3 0 0 1-3 3H8a3 3 0 0 1-3-3V9z"/><line x1="4" y1="9" x2="20" y2="9"/><path d="M5 10.3c-1.8-.2-2.8.5-2.8 1.7s1 2 2.8 1.9"/><path d="M19 10.3c1.8-.2 2.8.5 2.8 1.7s-1 2-2.8 1.9"/></svg>','#eee9f5','#5b4a8f'],
...
// "Kitchenware" (no "& Cookware") gets the same pot icon -- some categories
// get typed shorter than the canonical name, and this way either spelling
// still shows the real icon instead of falling back to the plain box.
CATICON['Kitchenware']=CATICON['Kitchenware & Cookware'];
...
document.getElementById('rv-go').addEventListener('click',function(){
  ...
  var d=new Date();
  var num=shortRef(c.kind==='transfer'?'MOV':'OUT');
